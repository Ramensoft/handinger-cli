...
import (
	...
	"github.com/stainless-sdks/handinger-cli/internal/autocomplete"
...
func init() {
	Command = &cli.Command{
		Name:      "handinger",
		Usage:     "CLI for the handinger API",
		Suggest:   true,
		Version:   Version,
		...
		Flags: []cli.Flag{
			...
			&requestflag.Flag[string]{
				Name:    "api-key",
				Sources: cli.EnvVars("HANDINGER_API_KEY"),
			},
		},
		Commands: []*cli.Command{
			...
			},
			{
				...
				UsageText:       "handinger @manpages [-o handinger.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "handinger.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "handinger.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
